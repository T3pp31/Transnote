...
2. DMG 内の **インストール.command** をダブルクリックします。
   - 初回実行時に「開発元を確認できない」などの警告が出る場合は、右クリック → **開く** を選んでください。
3. インストールが完了すると Transnote が起動します。
4. 必要に応じて DMG を取り出します。
...
| インストールスクリプトが実行できない | 右クリック → **開く** を選ぶ。それでも失敗する場合はターミナルから DMG 内の `インストール.command` を実行する |
