...
2. macOS の Gatekeeper により「開発元を確認できない」などの警告が出る場合:
   - システム設定 → プライバシーとセキュリティ で Transnote の起動を許可する
   - または、Finder でアプリを右クリック → **開く** を選択する
...
| 症状 | 対処 |
| --- | --- |
| アプリが開かない | 署名済み DMG から再インストールし、Gatekeeper の許可を確認する |
